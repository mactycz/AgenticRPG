...
def llm_engine(messages, stop_sequences=["Task"]) -> str:
    response = clientLLM.chat_completion(messages, stop=stop_sequences, max_tokens=1000)
    answer = response.choices[0].message.content
    return answer

agent = CodeAgent(tools=[], llm_engine=llm_engine, add_base_tools=True,additional_authorized_imports=['json'])
objects = agent.run(
    """Step 1: Generate text for a start for an rpg session in 200 words and add it to list that will be returned.
    ...
    Return should contain: story, character sheet, prompt - objects[0] should be the story, objects[1] should be list of characters, objects[2] should be prompt for image generation.
    Be very sure to provide correct code.
    """,
    return_code=True)
